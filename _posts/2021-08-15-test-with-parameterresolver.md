...
- resolveParamater() : 실제로 테스트 메서드에서 serves up an object of the correct type (a new Foo instance in this example), which will then be injected in your test method


진행하고 있는 프로젝트에 실제로 적용된 단위 테스트 코드이다.

UserDTOFixtureProvider에 UserDTO 픽스쳐를 생성하고

정상적으로 회원가입 할 수 있는 UserDTO 픽스쳐를 주입받아 테스트를 수행할 수 있다. 얼마나 심플한가?
