...
Lombok[^1]은 **컴파일타임**에 AST트리를 수정하여, 수정된 부분이 바이트코드에 반영된다. 좀 더 구체적으로 알아보면



1. javac는 소스파일을 파싱하여 AST트리[^3]를 만든다.
...
AnnotationProcessor은 Lombok의 어노테이션을 분석해서 AST트리를 수정하는 역할을 한다. 직접 코드를 확인해며 분석해보자.
...
다음으로  RoundEnvironment[^4] 인터페이스를 확인해보면
...
[^2]: Builder 패턴 : 객체 생성을 깔끔하고 유연하게 하기 위한 기법.
[^3]: AST 트리 : Abstract Syntax Tree의 약자 (추상구문트리) : 주로 컴파일러에서 프로그래밍 언어의 문법 및 각 문단의 역할 구조를 표현하기 위해서 사용된다. 특히 문법적으로 틀린 부분이 없는지, 소스코드 검사 단계에서 사용된다.
(추상이라는 말은 실제 구문에서 나타나는 모든 세세한 정보를 나타내지 않는다는 것을 의미함)
[^4]: RoundEnvironment인터페이스는 Annotation processing tool 프레임워크이고, 프로세서가 annotation processing의 라운드를 수정할 수 있도록 도와준다.