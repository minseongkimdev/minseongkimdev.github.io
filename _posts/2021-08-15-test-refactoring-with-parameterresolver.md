...
비록 지금 프로젝트가 구현단계이고 테스트 코드가 적을 때는 문제가 없을 수도 있지만, 프로젝트가 점점 거대해지고 테스트 코드가 많아짐에 따라 문제가 생길 수 있어 이 방법은 적절치 않다.
...
결합도가 높아짐에 따라 픽스쳐를 제공하는 클래스에 변경이 생기면이를 사용하는 쪽에서도 변경이 생길 수 있다.
